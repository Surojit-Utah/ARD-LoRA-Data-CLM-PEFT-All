...
class S2ClassDataset(DatasetBase):
    ...
    def deal_with_specical_datasets(self):
        ...
        for example in self.raw_dataset["train"]:
        ...
        for example in self.raw_dataset["validation"]:
            if len(example["choices"]["label"]) == 3:
        ...
        # Get total counts
        total_train = len(self.raw_dataset["train"])
        total_valid = len(self.raw_dataset["validation"])
        ...
        filtered_train = self.raw_dataset["train"].filter(
            lambda example: len(example["choices"]["label"]) == 4
        )

        # Filter the examples in the validation dataset
        filtered_valid = self.raw_dataset["validation"].filter(
            lambda example: len(example["choices"]["label"]) == 4
        )

        # Filter the examples in the test dataset
        filtered_test = self.raw_dataset["test"].filter(
            lambda example: len(example["choices"]["label"]) == 4
        )

        # Replace the original datasets with the filtered datasets
        self.raw_dataset["train"] = filtered_train
        self.raw_dataset["validation"] = filtered_valid
        self.raw_dataset["test"] = filtered_test

        print("====counts train====")
        print(f"Total number of training examples: {len(self.raw_dataset['train'])}")
        print("====counts valid====")
        print(
            f"Total number of validation examples: {len(self.raw_dataset['validation'])}"
        )
        ...
        self.raw_dataset["validation"] = self.raw_dataset["validation"].map(
            convert_choices_to_alpha
        )
        self.raw_dataset["test"] = self.raw_dataset["test"].map(
            convert_choices_to_alpha
        )
